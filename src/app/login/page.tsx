...
export default function LoginPage() {
    ...
    const handleLogin = async () => {
        ...
        try {
            ...
            const res = await api<LoginResponse>('/login', 'POST', { email, password });

            // Handle different response scenarios
            if (res.require2FA) {
                if (res.userId) {
                    localStorage.setItem('userId', res.userId); // Store userId for 2FA
                } else {
                    setError('User ID is missing for 2FA.');
                    setLoading(false);
                    return;
                }
                router.push('/verify-2fa'); // Redirect to 2FA verification
    ...
    return (
        <div className={` ${poppins.className} min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8`}>
            ...
            <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
                {/* White card with shadow containing the form */}
                        ...
                        <div>
                            ...
                            <div className="mt-1">
                                <input
                                    ...
                                    className="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm text-gray-700 placeholder-gray-700 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                                />
                            </div>
                        </div>
